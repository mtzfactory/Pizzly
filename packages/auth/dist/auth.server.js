...
class AuthServer {
    async setup(app) {
        ...
        app.route('/config/:providerConfigKey').delete(accessMiddleware.checkSecret.bind(accessMiddleware), configController.deleteProviderConfig.bind(configController));
        app.route('/connection/:connectionId').get(accessMiddleware.checkSecret.bind(accessMiddleware), connectionController.getConnectionCreds.bind(connectionController));
    }
}
